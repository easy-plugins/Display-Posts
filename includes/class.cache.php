...
class Cache {
	...
	public static function get( $key, $mode = 'transient', $group = self::PREFIX, $callback = NULL ) {
		...
		if ( apply_filters( 'Easy_Plugins\Display_Posts\Shortcode\Cache\Get', FALSE, $mode, $group_key . $key, $original_key, $group ) ) {

			$value = apply_filters( 'Easy_Plugins\Display_Posts\Shortcode\Cache\Get\Value', $value, $mode, $group_key . $key, $original_key, $group );
			...
			$value = wp_cache_get( $key, ( empty( $group ) ? 'ezpdp_cache' : $group ) );
			...
			if ( FALSE !== $pre ) {
				...
				$value   = wp_cache_get( $key, ( empty( $group ) ? 'ezpdp_option_cache' : $group ) );
				$timeout = wp_cache_get( '_timeout_' . $key, ( empty( $group ) ? 'ezpdp_option_cache' : $group ) );
				...
				if ( ! empty( $timeout ) && $timeout < time() ) {
					...
					if ( is_callable( $callback ) ) {
						...
						wp_cache_delete( $key, ( empty( $group ) ? 'ezpdp_option_cache' : $group ) );
						wp_cache_delete( '_timeout_' . $key, ( empty( $group ) ? 'ezpdp_option_cache' : $group ) );
					}
				}

			} else {

				$transient_option  = '_ezpdp_option_' . $key;
				$transient_timeout = '_ezpdp_option_timeout_' . $key;
		...
		$value = apply_filters( 'Easy_Plugins\Display_Posts\Shortcode\Cache\Get-' . $mode, $value, $original_key, $group );
	...
	public static function set( $key, $value, $expires = 0, $mode = 'transient', $group = self::PREFIX ) {
		...
		if ( apply_filters( 'Easy_Plugins\Display_Posts\Shortcode\Cache\Set', FALSE, $mode, $group_key . $key, $original_key, $value, $expires, $group ) ) {
			...
			wp_cache_set( $key, $value, ( empty( $group ) ? 'ezpdp_cache' : $group ), $expires );
			...
			if ( $_wp_using_ext_object_cache ) {
				...
				$result = wp_cache_set( $key, $value, ( empty( $group ) ? 'ezpdp_option_cache' : $group ) );

				if ( $expires ) {

					$result = wp_cache_set( '_timeout_' . $key, $expires, ( empty( $group ) ? 'ezpdp_option_cache' : $group ) );
				}

			} else {

				$transient_timeout = '_ezpdp_option_timeout_' . $key;
				$key               = '_ezpdp_option_' . $key;
		...
		do_action( 'Easy_Plugins\Display_Posts\Shortcode\Cache\Set-' . $mode, $original_key, $value, $expires, $group );
	...
	public static function clear( $key = TRUE, $mode = 'transient', $group = '' ) {
		...
		if ( apply_filters( 'Easy_Plugins\Display_Posts\Shortcode\Cache\Clear', FALSE, $mode, $full_key, $original_key, '', 0, $group ) ) {
			...
			if ( TRUE === $key ) {
				...
				wp_cache_delete( ( empty( $key ) ? 'ezpdp_cache' : $key ), 'ezpdp_cache' );
			}
			...
			if ( $_wp_using_ext_object_cache ) {
				...
				$result = wp_cache_delete( $key, ( empty( $group ) ? 'ezpdp_option_cache' : $group ) );

				wp_cache_delete( '_timeout_' . $key, ( empty( $group ) ? 'ezpdp_option_cache' : $group ) );

			} else {

				$option_timeout = '_ezpdp_option_timeout_' . $key;
				$option         = '_ezpdp_option_' . $key;
		...
		do_action( 'Easy_Plugins\Display_Posts\Shortcode\Cache\Clear-' . $mode, $original_key, $group );
 ...
 *
 * <code>
 * $fragment = new Fragment( 'unique-key', 3600 );
 *
 * if ( ! $fragment->get() ) {
 *
...
class Fragment {
	...
	public function get() {
		...
		$fragment = Cache::get( $this->key, 'transient', $this->group );
	...
	public function save( $ttl = NULL ) {
		...
		Cache::set( $this->key, ob_get_flush(), $ttl, 'transient', $this->group );
	}
	...
	public static function clear( $key, $group = '' ) {
		...
		if ( TRUE !== $key ) {
			...
			Cache::clear( $key, 'transient', self::PREFIX );

		} else {

			$group_key = empty( $group ) ? self::PREFIX : $group;

			Cache::clear( TRUE, 'transient', $group_key );
		}
